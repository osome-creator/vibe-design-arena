...
const fmtGrowth = (n: number | null | undefined) => {
  if (n == null) return "—";
  const cls = n >= 0 ? "swiss-growth-positive" : "swiss-growth-negative";
  const sign = n >= 0 ? "+" : "";
...
export default function Home() {
  ...
  if (loading) {
    return (
      <div className="swiss-loading">
        <div className="swiss-spinner" />
        <span>正在加载财务数据...</span>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="swiss-error">
        <div className="swiss-error-icon">!</div>
        <div className="swiss-error-title">数据加载失败</div>
        <div className="swiss-error-detail">{error || "未知错误"}</div>
        <div className="swiss-error-hint">
          请确认 API 路由 <code>/api/financial-data</code> 正常响应
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <header className="swiss-header">
        <span className="swiss-header-tag">财务分析报告</span>
        <h1 className="swiss-header-title">{m.title}</h1>
        <hr className="swiss-header-rule" />
        <div className="swiss-header-meta">
          <span>{m.company_name}</span>
          <span>股票代码 <strong>{m.stock_code}</strong></span>
          <span>{m.market}</span>
          <span>{m.author}</span>
          <span>报告期 <strong>{m.report_period}</strong></span>
          <span>预测期 <strong>{m.forecast_period}</strong></span>
        </div>
      </header>

      <main style={{ maxWidth: 960, margin: "0 auto", padding: "0 24px 80px 24px" }}>
        {/* ── Profitability ────────────────────────────── */}
        ...
        <Section title="现金流与成长性">
          ...
          {data.cash_flow_and_growth.note && (
            <p className="swiss-note">{data.cash_flow_and_growth.note}</p>
          )}
        </Section>
        ...
        <Section title="业务亮点">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
            {([
              ...
              <div key={i} className="swiss-kpi">
                <div className="swiss-kpi-label">{label}</div>
                <div className="swiss-kpi-value">{value}</div>
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section title="分析结论">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {data.conclusions.map((c, i) => (
              <div
                key={i}
                className={`swiss-conclusion-card ${c.risk_flag === true ? "risk" : c.risk_flag === false ? "positive" : "neutral"}`}
              >
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
                  <strong style={{ fontSize: 16, color: "var(--color-text-heading)" }}>{c.topic}</strong>
                  {c.risk_flag === true && (
                    <span className="swiss-badge-danger">风险信号</span>
                  )}
                  {c.risk_flag === false && (
                    <span className="swiss-badge-success">正面</span>
                  )}
                  {c.is_strategic && (
                    <span className="swiss-badge-strategic">战略级</span>
                  )}
                </div>
                <p style={{ fontSize: 14, color: "var(--color-text-body)", lineHeight: 1.7, margin: 0 }}>
                  {c.summary}
                </p>
        ...
        <Section title="2026-2028 财务预测">
          ...
          />
          <h4 style={{ fontSize: 16, fontWeight: 700, color: "var(--color-text-heading)", marginTop: 32, marginBottom: 12 }}>
            关键假设
          </h4>
          <ul className="swiss-assumption-list">
            {data.forecast.key_assumptions.map((a, i) => (
              <li key={i}>
                <span className="swiss-assumption-bullet">&#8201;&#8212;&#8201;</span>
                {a}
              </li>
            ))}
          </ul>
        ...
        <Section title="估值分析">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div className="swiss-valuation-card">
              <h4>{data.valuation.dcf_model.method}</h4>
              {[
                ...
                <div key={i} className="swiss-valuation-row">
                  <span>{label}</span>
                  <span>{val}</span>
                </div>
              ))}
            ...
            <div className="swiss-valuation-card">
              <h4>{data.valuation.pe_model.method}</h4>
              {[
        ...
        {/* ── Investment Opinion ───────────────────────── */}
        <Section title="投资建议">
          <div className="swiss-investment-card">
            <span className="swiss-badge-rating">{data.investment_opinion.rating}</span>
            <div style={{ marginTop: 24, textAlign: "left", maxWidth: 560, margin: "24px auto 0 auto" }}>
              <p style={{ fontSize: 14, lineHeight: 1.8, color: "var(--color-text-body)", margin: "0 0 12px 0" }}>
                <strong style={{ color: "var(--color-text-heading)" }}>短期建议：</strong>
                {data.investment_opinion.short_term_advice}
              </p>
              <p style={{ fontSize: 14, lineHeight: 1.8, color: "var(--color-text-body)", margin: "0 0 12px 0" }}>
                <strong style={{ color: "var(--color-text-heading)" }}>长期展望：</strong>
                {data.investment_opinion.long_term_view}
              </p>
              <p style={{ fontSize: 14, lineHeight: 1.8, color: "var(--color-text-body)", margin: 0 }}>
                <strong style={{ color: "var(--color-text-heading)" }}>升级触发条件：</strong>
                {data.investment_opinion.upgrade_trigger}
              </p>
            </div>
          </div>
        </Section>
        ...
        <Section title="风险因素">
          {data.risk_factors.map((r, i) => (
            <div key={i} className="swiss-risk-item">
              <span className="swiss-risk-marker">!</span>
              <div>
                <span className="swiss-risk-title">{r.risk}</span>
                <span className="swiss-risk-impact">{r.impact}</span>
              </div>
            </div>
          ))}
        </Section>
        ...
        {/* ── Footer ───────────────────────────────────── */}
        <footer className="swiss-footer" style={{ marginTop: 64 }}>
